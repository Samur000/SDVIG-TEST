...
import { HabitCard, CreateHabitModal, CelebrationModal } from './habits';
...
export function TasksPage() {
  ...
  const handleToggleHabit = (id: string) => {
    ...
    const wasCompleted = habit.records.includes(today);
    
    // Если отметили выполненным (не сняли галочку)
    if (!wasCompleted) {
      const newCount = habit.records.length + 1; // +1 потому что сейчас добавим сегодня
      
      dispatch({ type: 'TOGGLE_HABIT', payload: { id, date: today } });
      
      vibrate(50);
      
      // Проверяем, является ли новое количество кратным 5
  ...
  return (
    <Layout title="Дела">
      ...
      <CelebrationModal
        isOpen={celebrationCount !== null}
        count={celebrationCount || 0}
        onClose={() => setCelebrationCount(null)}
      />
    </Layout>
  );
}
